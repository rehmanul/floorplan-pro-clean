...
class ProfessionalCADProcessor {
    ...
    processDXF(filePath) {
        ...
        for (const p of polygons) {
            ...
            if (p.color === 1 || (p.layer && p.layer.includes('ENTRANCE')) || (p.layer && p.layer.includes('EXIT'))) {
                this.entrances.push({ polygon: p.polygon, layer: p.layer, color: p.color });
            } else if (p.color === 5 || (p.layer && (p.layer.includes('FORBIDDEN') || p.layer.includes('STAIR') || p.layer.includes('ELEVATOR')))) {
                this.forbiddenZones.push({ polygon: p.polygon, layer: p.layer, color: p.color });
            } else {
                // Walls as polygons (closed walls) or long segments are treated as wall polygons
...
ProfessionalCADProcessor.prototype.buildPolygonsFromSegments = function (segments) {
    ...
    for (let i = 0; i < segCount; i++) {
        ...
        while (true) {
            ...
            if (!found) break;
            // if closed loop
            if (Math.abs(chain[0].x - nextPt.x) < 1e-6 && Math.abs(chain[0].y - nextPt.y) < 1e-6) {
                // polygon closed
