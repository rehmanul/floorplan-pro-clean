    ...
    generateIlots(distribution = { '0-1': 10, '1-3': 25, '3-5': 30, '5-10': 35 }, totalIlots = 100) {
        ...
        for (const { range, count } of counts) {
            ...
            for (let i = 0; i < count; i++) {
                const area = minSize + this.rng() * Math.max(0, (maxSize - minSize));
                // create realistic rectangular shapes with some aspect ratio randomness
                const aspect = 0.5 + this.rng() * 1.5; // 0.5 - 2.0
                const width = Math.sqrt(area * aspect);
                const height = Math.max(0.1, area / width);
                placementCandidates.push({ area, width, height, id: `ilot_${placementCandidates.length + 1}` });
            }
        }
        ...
        for (const candidate of placementCandidates) {
            ...
            if (!pos) continue;
            const ilot = { id: candidate.id, x: pos.x, y: pos.y, width: candidate.width, height: candidate.height, candidate };
