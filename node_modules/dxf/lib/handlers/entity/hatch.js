...
var process = function process(tuples) {
  return tuples.reduce(function (entity, tuple) {
    ...
    switch (type) {
      ...
      case 10:
        {
          if (status === 'IDLE') entity.elevation.x = parseFloat(value);else if (status === 'POLYLINE' && isPolyline) {
            var point = {
              x: parseFloat(value),
              y: 0
            };
      ...
      case 20:
        {
          if (status === 'IDLE') entity.elevation.y = parseFloat(value);else if (status === 'POLYLINE' && isPolyline) {
            loop.entities[0].points[loop.entities[0].points.length - 1].y = parseFloat(value);
          } else if (status === 'SEED') {
      ...
      case 42:
        {
          if (isPolyline) loop.bulge = value;else fillDrawEntity(type, drawType, parseFloat(value));
        }
        break;
      ...
      case 73:
        {
          if (status === 'IDLE' || isPolyline) loop.closed = value;else fillDrawEntity(type, drawType, parseFloat(value));
        }
        break;
      ...
      case 75:
        {
          ...
          status = 'IDLE'; // 0 = Hatch “odd parity” area (Normal style)
...
function createDrawEntity(type) {
  if (isPolyline) return {}; // 1 = Line; 2 = Circular arc; 3 = Elliptic arc; 4 = Spline
